...
export class TelemetryService {
  ...
  async processTelemetryPayload(
    payload: CreateTelemetryPayloadDto,
  ): Promise<void> {
    const { session_id } = payload;
    try {
      if (payload.session_context) {
        await this.sessionContextRepo.save({
          session_id,
          ...payload.session_context,
        });
      }
      if (payload.decision_outcome) {
        await this.decisionOutcomeRepo.save({
          session_id,
          ...payload.decision_outcome,
        });
      }
      if (payload.social_context) {
        await this.socialContextRepo.save({
          session_id,
          ...payload.social_context,
        });
      }
      if (payload.dissemination) {
        await this.disseminationRepo.save({
          session_id,
      ...
      this.logger.error(
        `Error saving telemetry for session ${session_id}`,
        error.stack,
      );
    }
  }
