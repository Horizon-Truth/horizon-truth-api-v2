...
export class LanguagesService {
    ...
    async findAll(query: any): Promise<any> {
        ...
        if (isActive !== undefined) {
            queryBuilder.andWhere('lang.isActive = :isActive', { isActive: isActive === 'true' });
        }
    ...
    async create(createDto: CreateLanguageDto): Promise<Language> {
        ...
        if (existing) throw new ConflictException('Language with this name or code already exists');
    ...
    async update(id: string, updateDto: UpdateLanguageDto): Promise<Language> {
        ...
        if (updateDto.name || updateDto.code) {
            const existing = await this.languageRepository.createQueryBuilder('lang')
    ...
    async delete(id: string): Promise<void> {
        ...
        if (result.affected === 0) throw new NotFoundException('Language not found');
    }
}
