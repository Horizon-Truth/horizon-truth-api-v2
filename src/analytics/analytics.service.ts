...
export class AnalyticsService {
    ...
    return {
      ...
      distributions: {
        organizations: Object.fromEntries(
          orgTypes.map((t) => [t.status, parseInt(t.count)]),
        ),
        feedback: Object.fromEntries(
          feedbackStatus.map((s) => [s.status, parseInt(s.count)]),
        ),
      },
    };
  }
  ...
  async getGamePlayStats(orgId?: string) {
    const outcomeQuery = this.gameOutcomeRepository
      .createQueryBuilder('go')
      .leftJoin('scenarios', 's', 's.id = go.scenario_id');

    const recordQuery =
      this.playerScenarioRecordRepository.createQueryBuilder('psr');

    if (orgId) {
      outcomeQuery
        .innerJoin('organization_users', 'ou', 'ou.user_id = go.user_id')
        .where('ou.organization_id = :orgId', { orgId });
      recordQuery
        .innerJoin('organization_users', 'ou', 'ou.user_id = psr.user_id')
        .where('ou.organization_id = :orgId', { orgId });
    }
    ...
    const [
      ...
      outcomeQuery
        .clone()
        .select('COUNT(DISTINCT go.user_id)', 'count')
        .getRawOne()
        .catch(() => ({ count: 0 })),
      outcomeQuery
        .clone()
        .select('AVG(go.score)', 'avg')
        .getRawOne()
        .catch(() => ({ avg: 0 })),
      recordQuery
        .clone()
        .select('AVG(psr.best_accuracy_rate)', 'avg')
        .getRawOne()
        .catch(() => ({ avg: 0 })),
      recordQuery
        .clone()
        .select('AVG(CASE WHEN psr.is_completed THEN 100 ELSE 0 END)', 'rate')
        .getRawOne()
        .catch(() => ({ rate: 0 })),
    ]);
    ...
    const trendData = await this.gameOutcomeRepository
      .createQueryBuilder('go')
    ...
    // Scenario Popularity
    const popularityData = await this.gameOutcomeRepository
      .createQueryBuilder('go')
    ...
    const outcomeDist = await this.gameOutcomeRepository
      .createQueryBuilder('go')
    ...
    const difficultyDist = await this.gameOutcomeRepository
      .createQueryBuilder('go')
      .leftJoin('scenarios', 's', 's.id::text = go.scenario_id')
    ...
    return {
      overview: {
        ...
        completionRate: Math.round(
          parseFloat(completionRateData?.rate || '0') || 0,
        ),
      },
      ...
      distributions: {
        outcomes: Object.fromEntries(
          (outcomeDist || []).map((d) => [d.type, parseInt(d.count || '0')]),
        ),
  ...
  async getRecentSessions(orgId?: string, limit: number = 10) {
    const query = this.gameOutcomeRepository
      .createQueryBuilder('go')
      .leftJoin('scenarios', 's', 's.id::text = go.scenario_id')
