...
export class FeedbackService {
    ...
    async create(userId: string, createDto: CreateFeedbackDto): Promise<Feedback> {
    ...
    async findAll(query: FeedbackQueryDto): Promise<any> {
        const { scenarioId, status, priority, type, assignedTo, page = 1, limit = 10 } = query;
        ...
        if (scenarioId) {
            queryBuilder.andWhere('feedback.scenarioId = :scenarioId', { scenarioId });
        }
